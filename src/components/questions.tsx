...
import { GameData, Question, Activity, Round } from "./../data";
...
function Questions() {
  ...
  useEffect(() => {
    ...
    fetchQuestionaire()
      .then((fetchedData) => {
        if (actNum === 1) {
          setQuestion(fetchedData.activities[0].questions);
          setDisplay(fetchedData.activities[0]);
        } else {
          setQuestion(fetchedData.activities[actNum - 1]?.questions || []);
          setDisplay(fetchedData.activities[actNum - 1]);
        }
      })
      .catch((error) => {
        // Handle the error
